...
.
    ...
    urls = soup.find_all('div', itemtype = 'http://schema.org/ListItem')
    ...
    for items in prices:
        price = items.text.strip('$')
        price_lst.append(int(price))
    
    for item in urls:
        url = item.find('div', class_ = 'cy5jw6o dir dir-ltr')['aria-labelledby']
        titlecontents = url.split('_')
        newurl = titlecontents[1]
        id_lst.append(newurl)

    tup_lst = []
    for i in range(0,len(title_lst)):
        tup = (title_lst[i], price_lst[i], id_lst[i])
        tup_lst.append(tup)
...
.
    ...
    """
    base_path = os.path.abspath(os.path.dirname(__file__))
    full_path = os.path.join(base_path, "html_files/listing_" + listing_id + ".html")
    search_file = open(full_path, 'r', encoding = 'UTF-8')
    soup = BeautifulSoup(search_file, 'html.parser')
    pol_num = soup.find_all('li', class_ = 'f19phm7j dir dir-ltr')[0].text
    place_type = soup.find('h2', class_="_14i3z6h").text
    bedrooms = soup.find_all('li', class_ ="l7n4lsf dir dir-ltr")[1].text
    search_file.close()

    if 'studio' in bedrooms.lower():
        num = 1
    else:
        num = int(bedrooms.split(' ')[2])
    ...
    if 'pending' in pol_num.lower():
        number = 'Pending'
    elif 'exempt'in pol_num.lower() or 'not needed' in pol_num.lower():
        number = 'Exempt'
    else:
        number = pol_num.lstrip('Policy number: ')
    
    tup = (number, place, num)
...
def write_csv(data, filename):
    ...
    sort = sorted(data, key = lambda x: x[1], reverse=False)
...
def check_policy_numbers(data):
    ...
    for tuple in data:
        if invalid_Pol_Num in tuple:
            invalid_ids.append(tuple[2])
    return invalid_ids
...
def extra_credit(listing_id):
    ...
    """
    base_path = os.path.abspath(os.path.dirname(__file__))
    full_path = os.path.join(base_path, "html_files/listing_" + listing_id + "_reviews.html")
    review_file = open(full_path, 'r', encoding = 'UTF-8')
    soup = BeautifulSoup(review_file, 'html.parser')
    test = soup.find_all('li', class_ = '_1f1oir5')
    review_file.close()
    years = {}
    for i in test:
        dates = i.text
        years[dates[-4:]] = years.get(dates[-4:],0)+1
    count = 0
    for key in years:
        num_rev = years.get(key)
        if num_rev > 90:
            count += 1
        else:
            continue
